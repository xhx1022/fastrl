...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        # NOTE: Override the qkv projection for Eagle-3, Qwen 2.5 enables bias by default
        self.self_attn.q_proj = nn.Linear(config.hidden_size * 2, config.num_attention_heads * self.head_dim, bias=True)
        self.self_attn.k_proj = nn.Linear(config.hidden_size * 2, config.num_key_value_heads * self.head_dim, bias=True)
        self.self_attn.v_proj = nn.Linear(config.hidden_size * 2, config.num_key_value_heads * self.head_dim, bias=True)
