...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        self, micro_batch, temperature, calculate_entropy=False, return_hidden_states=False
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # Extract hidden states if requested (for remove_padding path)
                ...
                if self.use_ulysses_sp:
                    ...
                    # Gather hidden states across SP ranks and move to CPU immediately
                    if return_hidden_states and hidden_states_rmpad is not None:
                        hidden_states_rmpad = gather_outputs_and_unpad(
                            ...
                            unpad_dim=0,
                            padding_size=pad_size,
                        )  # (total_nnz, hidden_dim)
                        # Move to CPU immediately to avoid GPU OOM
                        hidden_states_rmpad = hidden_states_rmpad.detach().cpu()
                        hidden_states_indices = hidden_states_indices.cpu()
                ...
                # Return hidden states in rmpad format on CPU to avoid GPU OOM
                # Format: (rmpad_hidden_states, indices, (batch_size, seqlen))
                if return_hidden_states:
                    return entropy, log_probs, (hidden_states_rmpad, hidden_states_indices, hidden_states_shape_info)
            ...
            if return_hidden_states:
                # Get last layer hidden states from model output
                if hasattr(output, "hidden_states") and output.hidden_states is not None:
                    # hidden_states is a tuple of tensors, one per layer
                    # We want the last layer's hidden states
                    hidden_states = output.hidden_states[-1]  # (batch_size, seqlen, hidden_dim)
                else:
                    # If hidden_states not available, return None
                    hidden_states = None
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False, return_hidden_states=False) -> torch.Tensor:
            ...
            with torch.no_grad():
                ...
                if return_hidden_states:
                    ...
                    if hidden_states is not None:
                        # Handle two cases: tuple (rmpad format) or tensor (non-rmpad format)
                        if isinstance(hidden_states, tuple):
                            # rmpad format: (rmpad_hidden_states, indices, (batch_size, seqlen))
                            # Already on CPU, need to reconstruct per-sample
                            rmpad_hs, indices, (bsz, seqlen) = hidden_states
                            # Pad back to full shape on CPU per-sample to avoid GPU OOM
                            ...
                            for sample_idx in range(bsz):
                                hidden_states_lst.append(full_hs[sample_idx])  # Already on CPU
                        else:
                            # Regular tensor format (non-rmpad path)
                            # Move to CPU immediately per-sample
        ...
        if use_dynamic_bsz:
            ...
            # Restore hidden_states to original order (already on CPU)
            if return_hidden_states and hidden_states_lst:
                ...
                hidden_states_lst = [hidden_states_lst[i] for i in revert_indices]

        if return_hidden_states:
            return log_probs, entropys, hidden_states_lst
